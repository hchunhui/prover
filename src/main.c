...
void dump_prop(struct etree *p, int mode)
{
	const char *pc1, *pc2;
	switch(p->type)
	{
...
void __proof_imp(struct etree *p)
{
...
void __proof_not1(struct etree *p, int not)
{
	const char *pc1;
	switch(p->type)
	{
...
void __proof_not2(struct etree *p, int not)
{
	const char *pc1;
	switch(p->type)
	{
...
struct etree *simp_dist1(struct etree *t1, struct etree *t2)
{
	struct etree *t;
	if(t1->type == T_AND)
	{
...
struct etree *simp_dist(struct etree *p)
{
...
void __cons_clause(struct etree *p, struct list *list)
{
	switch(p->type)
	{
	case T_PROP:
...
struct list *cons_clause(struct etree *p)
{
...
int __proof_genor(struct etree *p, struct list *goal, int hc)
{
...
int __goal_include(struct etree *p, struct list *goal)
{
	switch(p->type)
	{
	case T_OR:
...
int __proof_dist(struct etree *p, int from, int hc, struct list *goal)
{
	int thc;
	switch(p->type)
	{
